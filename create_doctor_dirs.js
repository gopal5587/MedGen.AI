...
const directories = [
  path.join(baseDir, 'doctor'),
  path.join(baseDir, 'doctor', 'dashboard'),
  path.join(baseDir, 'doctor', 'components'),
  path.join(baseDir, 'doctor', 'pending-reviews'),
  path.join(baseDir, 'doctor', 'review', '[id]'),
  path.join(baseDir, 'doctor', 'approved'),
  path.join(baseDir, 'doctor', 'patients'),
  path.join(baseDir, 'doctor', 'profile'),
  path.join(baseDir, 'doctor', 'settings'),
  path.join(apiBaseDir, 'doctor'),
  path.join(apiBaseDir, 'doctor', 'pending-reviews'),
  path.join(apiBaseDir, 'doctor', 'stats'),
  path.join(apiBaseDir, 'doctor', 'approved'),
  path.join(apiBaseDir, 'doctor', 'review', '[id]')
];
...
function createFiles() {
  ...
  // 1. Doctor Layout
  files.push({
    path: path.join(baseDir, 'doctor', 'layout.tsx'),
...
`
  ...
  // 2. Doctor Sidebar
  files.push({
    path: path.join(baseDir, 'doctor', 'components', 'DoctorSidebar.tsx'),
    content: `"use client";
...
`
  ...
  // 3. API Stats Route
  files.push({
    path: path.join(apiBaseDir, 'doctor', 'stats', 'route.ts'),
...
`
  ...
  files.forEach(file => {
    try {
      ...
      console.log('Failed to create:', file.path, err.message);
    }
  });

  console.log('\nInitial files created! Now run the second script for remaining files.');
}
